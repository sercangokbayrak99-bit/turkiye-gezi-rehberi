...
type CityId = 'bursa' | 'istanbul' | 'ankara';
...
export default function App() {
  ...
  const favoritePlaces = [...allBursaPlaces, ...bursaBaths, ...istanbulFavoritePlaces, ...istanbulBeachFavoritePlaces, ...ankaraFavoritePlaces].filter(place => favorites.includes(place.id));
  ...
  const planCount = plannedDistricts.length + plannedSpiritual.length + plannedBursaPlaces.length + plannedIstanbulPlaces.length + plannedAnkaraPlaces.length;

  const content = tab === 'plan'
    ? <PlanScreen districtNames={plannedDistricts} spiritualIds={plannedSpiritual} bursaPlaceIds={plannedBursaPlaces} istanbulPlaceIds={plannedIstanbulPlaces} ankaraPlaceIds={plannedAnkaraPlaces} onRemoveDistrict={toggleDistrictPlan} onRemoveSpiritual={toggleSpiritualPlan} onRemoveBursaPlace={toggleBursaPlacePlan} onRemoveIstanbulPlace={toggleIstanbulPlacePlan} onRemoveAnkaraPlace={toggleAnkaraPlacePlan} />
    : tab === 'favorites'
  ...
  return (
    <View style={styles.app}>
      ...
      <PlaceModal place={selected} favorite={selected ? favorites.includes(selected.id) : false} planned={selected ? (selectedIsIstanbul ? plannedIstanbulPlaces : selectedIsAnkara ? plannedAnkaraPlaces : plannedBursaPlaces).includes(selected.id) : false} onClose={() => setSelected(null)} onFavorite={toggleFavorite} onTogglePlan={selectedIsIstanbul ? toggleIstanbulPlacePlan : selectedIsAnkara ? toggleAnkaraPlacePlan : toggleBursaPlacePlan} />
...
function Profile({ favoriteCount, planCount }: { favoriteCount: number; planCount: number }) {
  ...
  useEffect(() => {
    ...
    ]).then(([savedName, savedStyle, bursaSaved, istanbulSaved, ankaraSaved]) => {
...
function PlanScreen({ districtNames, spiritualIds, bursaPlaceIds, istanbulPlaceIds, ankaraPlaceIds, onRemoveDistrict, onRemoveSpiritual, onRemoveBursaPlace, onRemoveIstanbulPlace, onRemoveAnkaraPlace }: { districtNames: string[]; spiritualIds: string[]; bursaPlaceIds: string[]; istanbulPlaceIds: string[]; ankaraPlaceIds: string[]; onRemoveDistrict: (name: string) => void; onRemoveSpiritual: (id: string) => void; onRemoveBursaPlace: (id: string) => void; onRemoveIstanbulPlace: (id: string) => void; onRemoveAnkaraPlace: (id: string) => void }) {
  ...
  const destinations = [...districts.map(item => item.mapQuery), ...sites.map(item => item.mapQuery), ...bursaStops.map(item => item.mapQuery), ...istanbulStops.map(item => item.mapQuery), ...ankaraStops.map(item => item.mapQuery)];
  const openRoute = () => {
