...
function IzmirGuide({ plannedPlaceIds, favorites, onFavorite, onTogglePlan, onMenu }: { plannedPlaceIds: string[]; favorites: string[]; onFavorite: (id: string) => void; onTogglePlan: (id: string) => void; onMenu: () => void }) {
  ...
  const selectGuideSection = (section: CityGuideSection) => { if (section === 'beaches') { setShowBeachGuide(true); return; } setGuideSection(section); setQuery(''); setCategory('Tümü'); setTimeout(() => scrollRef.current?.scrollTo({ y: 0, animated: false }), 0); };
