...
import { ankaraAccommodations, ankaraDistricts, ankaraFamilyRoutes, ankaraFoodGuide, ankaraGallery, ankaraPlaces, ankaraRoutes, ankaraTransport, ankaraVenueAreas, type AnkaraDistrict, type AnkaraPlace } from './src/ankaraData';
import { izmirAccommodations, izmirDistricts, izmirFamilyRoutes, izmirFoodGuide, izmirGallery, izmirPlaces, izmirRoutes, izmirTransport, izmirVenueAreas, type IzmirDistrict, type IzmirPlace } from './src/izmirData';
...
const allIzmirPlanPlaces = [...izmirPlaces, ...izmirDistrictPlanPlaces];
const izmirPlaceIds = new Set(izmirPlaces.map(place => place.id));
...
export default function App() {
  ...
  const favoritePlaces = [...allBursaPlaces, ...bursaBaths, ...istanbulFavoritePlaces, ...istanbulBeachFavoritePlaces, ...ankaraFavoritePlaces, ...izmirFavoritePlaces].filter(place => favorites.includes(place.id));
...
function IzmirGuide({ plannedPlaceIds, favorites, onFavorite, onTogglePlan, onMenu }: { plannedPlaceIds: string[]; favorites: string[]; onFavorite: (id: string) => void; onTogglePlan: (id: string) => void; onMenu: () => void }) {
  const cityCategories: ExploreCategory[] = ['Tümü', 'Tarih', 'Doğa', 'Sahil', 'Lezzet', 'Manevi'];
  const [query, setQuery] = useState('');
  const [category, setCategory] = useState<ExploreCategory>('Tümü');
  ...
  return <SafeAreaView style={styles.istanbulPage}><ScrollView contentContainerStyle={styles.istanbulContent} showsVerticalScrollIndicator={false}>
    <ImageBackground source={require('./assets/izmir/hero.jpg')} style={styles.istanbulHero} imageStyle={styles.istanbulHeroImage}><View style={styles.istanbulHeroShade}/><View style={styles.istanbulHeroTop}><View style={styles.istanbulLive}><Text style={styles.istanbulLiveText}>YAYINDA · 35</Text></View><View style={styles.istanbulHeroActions}><Text style={styles.istanbulHeroRegion}>EGE · 30 İLÇE</Text><Pressable accessibilityRole="button" accessibilityLabel="Ana menüyü aç" onPress={onMenu} style={styles.roundButton}><Text style={styles.roundButtonText}>☰</Text></Pressable></View></View><View style={styles.istanbulHeroBody}><Text style={styles.istanbulHeroKicker}>EGE’NİN ÇOK KATMANLI ŞEHRİ</Text><Text style={styles.istanbulHeroTitle}>İzmir</Text><Text style={styles.istanbulHeroCopy}>Kemeraltı’ndan Efes’e, Bergama’dan Çeşme ve Karaburun koylarına uzanan kapsamlı Ege rehberi.</Text></View></ImageBackground>
    <View style={styles.istanbulBody}><View style={styles.citySearch}><Text style={styles.searchIcon}>⌕</Text><TextInput value={query} onChangeText={setQuery} placeholder="İlçe, antik kent, lezzet veya plaj ara" placeholderTextColor="#8A9691" style={styles.searchInput}/></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryRow}>{cityCategories.map(item=><Pressable key={item} onPress={()=>setCategory(item)} style={[styles.categoryChip,category===item&&styles.categoryChipActive]}><Text style={[styles.categoryText,category===item&&styles.categoryTextActive]}>{item}</Text></Pressable>)}</ScrollView>
    <View style={styles.moduleHeading}><View><Text style={styles.istanbulEyebrow}>İLÇE REHBERİ</Text><Text style={styles.moduleTitle}>30 ilçeyi keşfet</Text></View><Text style={styles.moduleHint}>{visibleDistricts.length} sonuç</Text></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.istanbulDistrictRail}>{visibleDistricts.map((item,index)=>{const id=`izmir-ilce-${item.name}`;const planned=plannedPlaceIds.includes(id);return <Pressable key={item.name} onPress={()=>setSelectedDistrict(item)} style={[styles.istanbulDistrictCard,{backgroundColor:index%3===0?'#315F53':index%3===1?'#75513B':'#477A89'}]}><Text style={styles.istanbulDistrictSide}>İZMİR · İLÇE</Text><Text style={styles.istanbulDistrictName}>{item.name}</Text><Text style={styles.istanbulDistrictCopy}>{item.signature}</Text><Text style={styles.istanbulDistrictOpen}>Detayı aç  →</Text><Pressable onPress={event=>{event.stopPropagation();onTogglePlan(id);}} style={[styles.istanbulDistrictPlan,planned&&styles.istanbulDistrictPlanActive]}><Text style={styles.istanbulDistrictPlanText}>{planned?'✓  Planımda':'+  Planıma ekle'}</Text></Pressable></Pressable>;})}</ScrollView>
    {category!=='Lezzet'&&<>{renderPlaces('Antik kentler & tarihî doku','İZMİR’İN HAFIZASI',history)}{renderPlaces('Manevi duraklar','İNANÇ MİRASI',spiritual)}{renderPlaces('Doğa & aile keşifleri','DELTA, VADİ & YEŞİL',nature)}{renderPlaces('Sahiller & plajlar','EGE KIYILARI',coast)}</>}
