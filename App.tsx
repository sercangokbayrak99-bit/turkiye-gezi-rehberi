...
import { izmirDistrictGuideByName, type IzmirDistrictTheme } from './src/izmirDistrictGuides';
...
const izmirTheme = {
  forest: '#173F35', forestDark: '#102F28', moss: '#376A5B',
  cream: '#F4EFE3', creamSoft: '#F8F5ED', paper: '#FFFDF8', white: '#FFFFFF',
  gold: '#C99A4A', goldSoft: '#E6D3A7', ink: '#18332C', muted: '#6F7E78',
  line: '#DED9CC', lineGreen: '#CBD8D1', seaSoft: '#DCEBE7', sandSoft: '#EFE1C8', sageSoft: '#DFE8D8',
} as const;
...
type IzmirDistrictSection = 'overview' | 'places' | 'beaches' | 'food' | 'venues' | 'nightlife' | 'shopping' | 'nature' | 'family' | 'photos' | 'routes' | 'practical';
const izmirGuideSections: { id: CityGuideSection; label: string; icon: string }[] = [
  { id: 'overview', label: 'Genel Bakış', icon: '⌂' }, { id: 'places', label: 'Gezilecek Yerler', icon: '◇' }, { id: 'food', label: 'Yeme & İçme', icon: '♨' },
  { id: 'beaches', label: 'Sahiller & Plajlar', icon: '≈' }, { id: 'stay-routes', label: 'Konaklama & Rotalar', icon: '⌑' }, { id: 'services', label: 'Ulaşım & Hizmetler', icon: '↔' },
];
...
export default function App() {
  ...
  const content = tab === 'plan'
    ...
    : tab === 'profile'
      ...
      : activeCity === 'izmir'
        ? <IzmirGuide plannedPlaceIds={plannedIzmirPlaces} favorites={favorites} onFavorite={toggleFavorite} onTogglePlan={toggleIzmirPlacePlan} onMenu={() => setMenuOpen(true)} onNavigate={setTab} />
      : <MainContent
          exploreOnly={false}
          query={query}
          setQuery={setQuery}
  ...
  return (
    <View style={styles.app}>
      ...
      {content}
      <BottomTabs tab={tab} setTab={setTab} favoriteCount={favorites.length} planCount={planCount} izmirThemed={activeCity === 'izmir'} />
...
function IzmirGuide({ plannedPlaceIds, favorites, onFavorite, onTogglePlan, onMenu, onNavigate }: { plannedPlaceIds: string[]; favorites: string[]; onFavorite: (id: string) => void; onTogglePlan: (id: string) => void; onMenu: () => void; onNavigate: (tab: Tab) => void }) {
  ...
  const [placeFocus, setPlaceFocus] = useState<'all' | 'family'>('all');
  const [showAllDistricts, setShowAllDistricts] = useState(false);
  const [districtQuery,setDistrictQuery]=useState('');
  const [districtFilter,setDistrictFilter]=useState<'Tümü'|IzmirDistrictTheme>('Tümü');
  const [query, setQuery] = useState('');
  const [category, setCategory] = useState<ExploreCategory>('Tümü');
  ...
  const featuredDistricts = [
    ['Konak','izmir-saat-kulesi','Tarih · Kemeraltı · Kordon'], ['Çeşme','izmir-cesme-kalesi','Deniz · Plaj · Gece hayatı'],
    ['Foça','izmir-foca','Sahil · Eski Foça · Gün batımı'], ['Seferihisar','izmir-sigacik','Sığacık · Teos · Sakin şehir'],
    ['Selçuk','izmir-efes','Tarih · Kültür · Efes'], ['Bergama','izmir-bergama','UNESCO · Arkeoloji · Çarşı'],
    ['Karaburun','izmir-karaburun','Koylar · Doğa · Nergis'], ['Çiğli','izmir-gediz','Gediz Deltası · Aile · Doğa'],
  ].map(([name,placeId,tags])=>({district:izmirDistricts.find(item=>item.name===name)!,place:izmirPlaces.find(item=>item.id===placeId)!,tags}));
  const overviewDistricts=[...izmirDistricts].sort((a,b)=>a.name.localeCompare(b.name,'tr')).filter(item=>{const profile=izmirDistrictGuideByName.get(item.name);const text=`${item.name} ${item.signature} ${item.highlights.join(' ')} ${item.flavors.join(' ')}`.toLocaleLowerCase('tr-TR');return(!districtQuery.trim()||text.includes(districtQuery.trim().toLocaleLowerCase('tr-TR')))&&(districtFilter==='Tümü'||profile?.themes.includes(districtFilter));});
  const factIcons = ['◉', '▥', '⌖', '≈', '◇', '▣'];
  const intentCards = [
    { id: 'history' as const, icon: '◇', title: 'Tarih & Kültür', copy: 'Konak, Selçuk, Bergama ve daha fazlası', cardStyle: styles.izmirIntentHistory, textStyle: styles.izmirIntentHistoryText, copyStyle: styles.izmirIntentHistoryCopy },
    { id: 'sea' as const, icon: '≈', title: 'Deniz & Koylar', copy: 'Çeşme, Seferihisar, Foça ve saklı koylar', cardStyle: styles.izmirIntentSea, textStyle: styles.izmirIntentSeaText, copyStyle: styles.izmirIntentSeaCopy },
    { id: 'family' as const, icon: '◉', title: 'Ailece Keşif', copy: 'Sasalı, İnciraltı ve çocuk dostu rotalar', cardStyle: styles.izmirIntentFamily, textStyle: styles.izmirIntentFamilyText, copyStyle: styles.izmirIntentFamilyCopy },
    { id: 'evening' as const, icon: '☾', title: 'Akşam & Sosyal', copy: 'Alsancak, Bostanlı, Alaçatı ve daha fazlası', cardStyle: styles.izmirIntentEvening, textStyle: styles.izmirIntentEveningText, copyStyle: styles.izmirIntentEveningCopy },
  ];
  const selectGuideSection = (section: CityGuideSection) => { if (section === 'beaches') { setShowBeachGuide(true); return; } if (section === 'food') setFoodSection('venues'); setPlaceFocus('all'); setGuideSection(section); setQuery(''); setCategory('Tümü'); setTimeout(() => scrollRef.current?.scrollTo({ y: 0, animated: false }), 0); };
  ...
  const selectIntent = (intent: 'history' | 'sea' | 'family' | 'evening') => {
    if (intent === 'sea') { selectGuideSection('beaches'); return; }
    if (intent === 'evening') { setGuideSection('food'); setFoodSection('nightlife'); setPlaceFocus('all'); setQuery(''); setCategory('Tümü'); }
    else { setGuideSection('places'); setPlaceFocus(intent === 'family' ? 'family' : 'all'); setCategory(intent === 'history' ? 'Tarih' : 'Tümü'); setQuery(''); }
  ...
  return <SafeAreaView style={[styles.istanbulPage,styles.izmirPage]}><ScrollView ref={scrollRef} contentContainerStyle={[styles.istanbulContent,styles.izmirContent,isDesktop&&styles.izmirContentDesktop,isTablet&&styles.izmirContentTablet]} showsVerticalScrollIndicator={false}>
    <View style={[styles.izmirHeader,isDesktop&&styles.izmirHeaderDesktop]}>{isDesktop?<><View style={styles.izmirBrand}><Text style={styles.izmirBrandMark}>✥</Text><Text style={styles.izmirBrandText}>Türkiye Gezi Rehberi</Text></View><View style={styles.izmirHeaderActions}><Pressable onPress={()=>onNavigate('favorites')} style={styles.izmirHeaderLink}><Text style={styles.izmirHeaderLinkIcon}>♡</Text><Text style={styles.izmirHeaderLinkText}>Favorilerim</Text></Pressable><Pressable onPress={()=>onNavigate('plan')} style={styles.izmirHeaderLink}><Text style={styles.izmirHeaderLinkIcon}>⌑</Text><Text style={styles.izmirHeaderLinkText}>Gezi Planım</Text></Pressable><Pressable accessibilityRole="button" accessibilityLabel="Ana menüyü aç" onPress={onMenu} style={styles.izmirHeaderMenu}><Text style={styles.izmirHeaderMenuText}>☰</Text></Pressable></View></>:<><Pressable accessibilityRole="button" accessibilityLabel="Ana menüyü aç" onPress={onMenu} style={styles.izmirHeaderMenu}><Text style={styles.izmirHeaderMenuText}>☰</Text></Pressable><View style={styles.izmirBrand}><Text style={styles.izmirBrandMark}>✥</Text><Text numberOfLines={1} style={styles.izmirBrandText}>Türkiye Gezi Rehberi</Text></View><Pressable onPress={()=>onNavigate('favorites')} style={styles.izmirHeaderMenu}><Text style={styles.izmirHeaderHeart}>♡</Text></Pressable></>}</View>
    <View style={[styles.istanbulHero,styles.izmirPremiumHero,isDesktop?styles.izmirPremiumHeroDesktop:styles.izmirPremiumHeroMobile]}>{Platform.OS === 'web' ? <View style={[StyleSheet.absoluteFill, styles.istanbulHeroImage, ({ backgroundImage: `url("${require('./assets/izmir/hero.jpg').uri}")`, backgroundPosition: '25% center', backgroundRepeat: 'no-repeat', backgroundSize: 'cover' } as object)]}/> : <Image source={require('./assets/izmir/hero.jpg')} resizeMode="cover" style={StyleSheet.absoluteFill}/>}<View style={[styles.istanbulHeroShade,styles.izmirPremiumHeroShade]}/><View style={styles.izmirHeroLeftGradient}/><View style={[styles.istanbulHeroBody,styles.izmirPremiumHeroBody]}><Text style={[styles.istanbulHeroTitle,isDesktop&&styles.izmirPremiumHeroTitle]}>İZMİR</Text><Text style={styles.izmirHeroSubtitle}>Ege’nin İncisi</Text><Text style={[styles.istanbulHeroCopy,styles.izmirPremiumHeroCopy]}>Tarih, deniz, kültür ve Ege yaşamının buluştuğu şehir.</Text></View></View>
    <View style={[styles.izmirGuideNav,isDesktop&&styles.izmirGuideNavDesktop]}><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[styles.izmirGuideNavRail,isDesktop&&styles.izmirGuideNavRailDesktop]}>{izmirGuideSections.map(item=><Pressable key={item.id} onPress={()=>selectGuideSection(item.id)} style={[styles.izmirGuideNavItem,isDesktop&&styles.izmirGuideNavItemDesktop,guideSection===item.id&&styles.izmirGuideNavItemActive]}><Text style={[styles.izmirGuideNavIcon,guideSection===item.id&&styles.izmirGuideNavTextActive]}>{item.icon}</Text><Text numberOfLines={1} style={[styles.izmirGuideNavText,isDesktop&&styles.izmirGuideNavTextDesktop,guideSection===item.id&&styles.izmirGuideNavTextActive]}>{item.label}</Text></Pressable>)}</ScrollView></View>
    <View style={[styles.istanbulBody,styles.izmirPremiumBody]}>
      {guideSection!=='overview'&&<View style={styles.izmirBreadcrumb}><Pressable onPress={()=>selectGuideSection('overview')} style={styles.izmirBreadcrumbBack}><Text numberOfLines={1} style={styles.izmirBreadcrumbBackText}>←<Text style={styles.izmirBreadcrumbBackGap}>  </Text>Genel Bakış</Text></Pressable><Text style={styles.izmirBreadcrumbCurrent}><Text style={styles.izmirBreadcrumbCity}>İzmir</Text><Text style={styles.izmirBreadcrumbSeparator}>  ›  </Text><Text style={styles.izmirBreadcrumbActive}>{izmirGuideSections.find(item=>item.id===guideSection)?.label}</Text></Text></View>}
      {guideSection==='overview'&&<><View style={styles.izmirOverviewIntro}><View style={styles.izmirOverviewTitleRow}><View style={styles.izmirOverviewLine}/><View style={styles.izmirOverviewDot}/><Text style={[styles.moduleTitle,styles.izmirHeadingText,isDesktop&&styles.izmirOverviewTitleDesktop]}>İzmir’e Genel Bakış</Text><View style={styles.izmirOverviewDot}/><View style={styles.izmirOverviewLine}/></View><Text style={styles.izmirOverviewCopy}>Ege’nin tarihini, kıyılarını, mutfağını ve şehir yaşamını tek rehberde keşfet.</Text></View>
        <View style={[styles.izmirDashboard,isDesktop&&styles.izmirDashboardDesktop]}>
          <View style={[styles.izmirDashboardPanel,styles.izmirFactsPanel,isDesktop&&styles.izmirDashboardFacts,isTablet&&styles.izmirDashboardTabletHalf]}><Text style={styles.izmirPanelEyebrow}>HIZLI BİLGİLER</Text><View style={styles.izmirFactsGrid}>{izmirQuickFacts.map((fact,index)=><View key={fact.label} style={[styles.izmirFact,{backgroundColor:[izmirTheme.creamSoft,izmirTheme.cream,izmirTheme.sageSoft,izmirTheme.creamSoft,izmirTheme.sandSoft,izmirTheme.sageSoft][index%6]}]}><Text style={styles.izmirFactIcon}>{factIcons[index]??'•'}</Text><View style={styles.izmirFactBody}><Text numberOfLines={1} adjustsFontSizeToFit style={styles.izmirFactValue}>{fact.label==='Nüfus'?'4,5 M+':fact.value}</Text><Text style={styles.izmirFactLabel}>{fact.label}</Text></View></View>)}</View><Text style={styles.izmirFactsFootnote}>Kaynak: TÜİK, UNESCO ve mevcut İzmir rehber envanteri.</Text></View>
          <View style={[styles.izmirDashboardPanel,styles.izmirFeaturedPanel,isDesktop&&styles.izmirDashboardFeatured,isTablet&&styles.izmirDashboardTabletHalf]}><Text style={styles.izmirFeaturedEyebrow}>★  ÖNE ÇIKANLAR</Text>{isDesktop||isTablet?<View style={styles.izmirFeaturedGrid}>{featuredPlaces.map(place=><Pressable key={place.id} onPress={()=>setSelectedPlace(place)} style={styles.izmirFeaturedCard}><Image source={place.image} style={styles.izmirFeaturedImage}/><View style={styles.izmirFeaturedShade}/><View style={styles.izmirFeaturedBody}><Text style={styles.izmirFeaturedDistrict}>{place.district.toUpperCase()}</Text><Text style={styles.izmirFeaturedName}>{place.name}</Text></View></Pressable>)}</View>:<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.izmirFeaturedRail}>{featuredPlaces.map(place=><Pressable key={place.id} onPress={()=>setSelectedPlace(place)} style={[styles.izmirFeaturedCard,styles.izmirFeaturedCardMobile]}><Image source={place.image} style={styles.izmirFeaturedImage}/><View style={styles.izmirFeaturedShade}/><View style={styles.izmirFeaturedBody}><Text style={styles.izmirFeaturedDistrict}>{place.district.toUpperCase()}</Text><Text style={styles.izmirFeaturedName}>{place.name}</Text></View></Pressable>)}</ScrollView>}<Pressable onPress={()=>selectGuideSection('places')} style={styles.izmirFeaturedAll}><Text style={styles.izmirFeaturedAllText}>Tüm öne çıkanları gör  →</Text></Pressable></View>
        </View>
        <View style={styles.izmirGuideHeading}><Text style={[styles.moduleTitle,styles.izmirHeadingText,isDesktop&&styles.izmirGuideHeadingTitle]}>İzmir’i Nasıl Keşfetmek İstersin?</Text></View><View style={[styles.izmirIntentGrid,isDesktop&&styles.izmirIntentGridDesktop]}>{intentCards.map(item=><Pressable key={item.id} onPress={()=>selectIntent(item.id)} style={[styles.izmirIntentCard,item.cardStyle]}><View style={styles.izmirIntentIcon}><Text style={[styles.izmirIntentIconText,item.textStyle]}>{item.icon}</Text></View><View style={styles.izmirIntentBody}><Text style={[styles.izmirIntentTitle,item.textStyle]}>{item.title}</Text><Text numberOfLines={2} style={[styles.izmirIntentCopy,item.copyStyle]}>{item.copy}</Text></View><Text style={[styles.izmirIntentArrow,item.textStyle]}>→</Text></Pressable>)}</View>
        <View style={styles.izmirDistrictSection}>
          <Text style={[styles.moduleTitle,styles.izmirHeadingText,isDesktop&&styles.izmirGuideHeadingTitle]}>İzmir İlçeleri</Text>
          <Text style={styles.izmirDistrictIntro}>30 ilçeyi keşfet; gezilecek yerleri, sahilleri, lezzetleri ve öne çıkan rotaları incele.</Text>
          <View style={styles.izmirDistrictSectionTop}><Text style={styles.izmirPanelEyebrow}>ÖNE ÇIKAN İLÇELER</Text><Text style={styles.izmirDistrictCount}>8 seçki</Text></View>
          <View style={[styles.izmirFeaturedDistrictGrid,isDesktop&&styles.izmirFeaturedDistrictGridDesktop]}>{featuredDistricts.map(item=><Pressable key={item.district.name} onPress={()=>setSelectedDistrict(item.district)} style={[styles.izmirFeaturedDistrictCard,isTablet&&styles.izmirFeaturedDistrictCardTablet,isDesktop&&styles.izmirFeaturedDistrictCardDesktop]}><Image source={item.place.image} style={styles.izmirFeaturedDistrictImage} {...(Platform.OS==='web'?({loading:'lazy'} as object):{})}/><View style={styles.izmirFeaturedDistrictShade}/><View style={styles.izmirFeaturedDistrictBody}><Text style={styles.izmirFeaturedDistrictName}>{item.district.name.toUpperCase()}</Text><Text numberOfLines={1} style={styles.izmirFeaturedDistrictTags}>{item.tags}</Text></View><Text style={styles.izmirFeaturedDistrictArrow}>→</Text></Pressable>)}</View>
          <Pressable onPress={()=>setShowAllDistricts(value=>!value)} style={styles.izmirAllDistrictsToggle}><Text style={styles.izmirAllDistrictsToggleText}>{showAllDistricts?'Tüm ilçeleri kapat  ↑':'Tüm 30 ilçeyi keşfet  →'}</Text></Pressable>
          {showAllDistricts&&<View><Text style={styles.izmirAllDistrictsTitle}>Tüm İzmir İlçeleri</Text><View style={styles.izmirDistrictSearch}><Text style={styles.searchIcon}>⌕</Text><TextInput value={districtQuery} onChangeText={setDistrictQuery} placeholder="İlçe, deneyim veya lezzet ara" placeholderTextColor={izmirTheme.muted} style={styles.searchInput}/></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.izmirDistrictFilterRail}>{(['Tümü','Deniz','Tarih','Doğa','Gastronomi','Aile','Eğlence'] as const).map(filter=><Pressable key={filter} onPress={()=>setDistrictFilter(filter)} style={[styles.izmirDistrictFilterChip,districtFilter===filter&&styles.izmirDistrictFilterChipActive]}><Text style={[styles.izmirDistrictFilterText,districtFilter===filter&&styles.izmirDistrictFilterTextActive]}>{filter==='Tümü'?'Tümü':`${districtThemeIcon(filter)} ${filter}`}</Text></Pressable>)}</ScrollView><Text style={styles.izmirDistrictFilterResult}>{overviewDistricts.length} ilçe</Text><View style={[styles.izmirAllDistrictsGrid,isTablet&&styles.izmirAllDistrictsGridTablet,isDesktop&&styles.izmirAllDistrictsGridDesktop]}>{overviewDistricts.map(district=><Pressable key={district.name} onPress={()=>setSelectedDistrict(district)} style={[styles.izmirCompactDistrictCard,isTablet&&styles.izmirCompactDistrictCardTablet,isDesktop&&styles.izmirCompactDistrictCardDesktop]}><Text style={styles.izmirCompactDistrictIcon}>◇</Text><Text numberOfLines={1} style={styles.izmirCompactDistrictName}>{district.name}</Text><Text style={styles.izmirCompactDistrictArrow}>→</Text></Pressable>)}</View>{!overviewDistricts.length&&<View style={styles.izmirDistrictEmpty}><Text style={styles.izmirDistrictEmptyTitle}>Bu filtrede ilçe bulunamadı</Text><Text style={styles.izmirDistrictEmptyCopy}>Arama metnini veya deneyim filtresini değiştirmeyi deneyin.</Text></View>}</View>}
        </View>
      </>}
      {guideSection==='places'&&<><View style={styles.citySearch}><Text style={styles.searchIcon}>⌕</Text><TextInput value={query} onChangeText={setQuery} placeholder="Mekân, ilçe veya deneyim ara" placeholderTextColor="#8A9691" style={styles.searchInput}/></View>{placeFocus==='all'&&<><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryRow}>{placeCategories.map(item=><Pressable key={item} onPress={()=>setCategory(item)} style={[styles.categoryChip,category===item&&styles.categoryChipActive]}><Text style={[styles.categoryText,category===item&&styles.categoryTextActive]}>{item}</Text></Pressable>)}</ScrollView>{renderPlaces('Antik kentler & tarihî doku','İZMİR’İN HAFIZASI',history)}{renderPlaces('Manevi duraklar','İNANÇ MİRASI',spiritual)}{renderPlaces('Doğa & aile keşifleri','DELTA, VADİ & YEŞİL',nature)}{renderPlaces('Kent sahilleri & kıyı rotaları','EGE KIYILARI',coast)}</>}{(placeFocus==='family'||category==='Tümü')&&<><View style={styles.moduleHeading}><View><Text style={styles.istanbulEyebrow}>ÇOCUKLARLA İZMİR</Text><Text style={styles.moduleTitle}>Aile & çocuk rotaları</Text></View><Text style={styles.moduleHint}>{izmirFamilyRoutes.length} rota</Text></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.venueRail}>{izmirFamilyRoutes.map(item=><View key={item.area} style={styles.venueCard}><Text style={styles.venueDistrict}>{item.district.toUpperCase()}</Text><Text style={styles.venueArea}>{item.area}</Text><Text style={styles.venueCharacter}>{item.character}</Text><Pressable onPress={()=>openMap(item.mapQuery)} style={[styles.venueButton,styles.venueButtonDark]}><Text style={[styles.venueButtonText,styles.venueButtonTextDark]}>Rotayı haritada aç  ↗</Text></Pressable></View>)}</ScrollView></>}</>}
...
function IzmirDistrictModal({ district, planned, plannedPlaceIds, favorites, onFavorite, onTogglePlan, onClose }: { district: IzmirDistrict | null; planned: boolean; plannedPlaceIds: string[]; favorites: string[]; onFavorite: (id:string)=>void; onTogglePlan: (id:string)=>void; onClose:()=>void }) {
  const { width } = useWindowDimensions();
  const [section,setSection]=useState<IzmirDistrictSection>('overview');
  const [selectedPlace,setSelectedPlace]=useState<IzmirPlace|null>(null);
  const [selectedBeach,setSelectedBeach]=useState<IzmirBeach|null>(null);
  useEffect(()=>{setSection('overview');setSelectedPlace(null);setSelectedBeach(null);},[district?.name]);
  if(!district)return <Modal visible={false}/>;

  const profile=izmirDistrictGuideByName.get(district.name);
  const places=izmirPlaces.filter(place=>place.district===district.name);
  const beaches=izmirBeaches.filter(beach=>beach.district===district.name);
  const foods=izmirFoodGuide.filter(item=>item.area.toLocaleLowerCase('tr-TR').includes(district.name.toLocaleLowerCase('tr-TR')));
  const venues=izmirVenueAreas.filter(item=>item.district===district.name);
  const nightlife=izmirNightlifeAreas.filter(item=>item.district===district.name);
  const shopping=izmirShoppingStreets.filter(item=>item.district===district.name);
  const familyRoutes=izmirFamilyRoutes.filter(item=>item.district===district.name);
  const naturePlaces=places.filter(place=>['Doğa','Manzara','Aile & Park','Sahil'].includes(place.category));
  const familyPlaces=places.filter(place=>place.category==='Aile & Park');
  const heroPlace=(profile?.photoPlaceId?izmirPlaces.find(place=>place.id===profile.photoPlaceId):undefined)??places[0];
  const themes=profile?.themes??[];
  const openSearch=(query:string)=>Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${query} ${district.name} İzmir`)}`);
  const openDistrictRoute=(stops:string[])=>{if(stops.length<2){openSearch(stops[0]??'gezilecek yerler');return;}Linking.openURL(`https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(`${stops[0]} ${district.name} İzmir`)}&destination=${encodeURIComponent(`${stops[stops.length-1]} ${district.name} İzmir`)}&waypoints=${encodeURIComponent(stops.slice(1,-1).map(stop=>`${stop} ${district.name} İzmir`).join('|'))}`);};
  const services:Array<[string,string,string]>=[['☕','Kafeler','kafeler'],['🍴','Restoranlar','restoranlar'],['⌂','Konaklama','oteller'],['H','Hastaneler','hastaneler'],['+','Eczaneler','eczaneler'],['P','Otopark','otoparklar'],['↔','Ulaşım','toplu taşıma']];
  const tabs:Array<{id:IzmirDistrictSection;label:string;icon:string}>=[
    {id:'overview',label:'Genel Bakış',icon:'✨'},
    {id:'places',label:'Gezilecek Yerler',icon:'📍'},
    ...(beaches.length?[{id:'beaches' as const,label:'Plajlar & Koylar',icon:'🏖️'}]:[]),
    {id:'food',label:'Ne Yenir?',icon:'🍴'},
    {id:'venues',label:'Kafe & Restoran',icon:'☕'},
    ...(nightlife.length?[{id:'nightlife' as const,label:'Gece Hayatı',icon:'🌙'}]:[]),
    ...(shopping.length?[{id:'shopping' as const,label:'Alışveriş',icon:'🛍️'}]:[]),
    ...(themes.includes('Doğa')||naturePlaces.length?[{id:'nature' as const,label:'Doğa',icon:'🌿'}]:[]),
    ...(themes.includes('Aile')||familyRoutes.length||familyPlaces.length?[{id:'family' as const,label:'Ailece',icon:'👨‍👩‍👧'}]:[]),
    ...(heroPlace||places.length?[{id:'photos' as const,label:'Fotoğraf Noktaları',icon:'📸'}]:[]),
    {id:'routes',label:'Hazır Rotalar',icon:'🗺️'},
    {id:'practical',label:'Pratik Bilgiler',icon:'ℹ️'},
  ];
  const renderPlaceCards=(items:IzmirPlace[])=>items.length?<View style={styles.izmirDistrictPlaceGrid}>{items.map(place=><View key={place.id} style={[styles.istanbulPlaceCard,styles.izmirDistrictPlaceCard]}><Pressable onPress={()=>setSelectedPlace(place)}><View><Image source={place.image} style={styles.istanbulPlaceImage} {...(Platform.OS==='web'?({loading:'lazy'} as object):{})}/><Pressable hitSlop={10} onPress={event=>{event.stopPropagation();onFavorite(place.id);}} style={styles.favoriteButton}><Text style={[styles.favoriteIcon,favorites.includes(place.id)&&styles.favoriteIconActive]}>{favorites.includes(place.id)?'♥':'♡'}</Text></Pressable></View><View style={styles.istanbulPlaceBody}><Text style={styles.istanbulPlaceMeta}>{place.category.toUpperCase()} · {place.district.toUpperCase()}</Text><Text style={styles.istanbulPlaceName}>{place.name}</Text><Text numberOfLines={3} style={styles.istanbulPlaceCopy}>{place.summary}</Text><Text style={styles.istanbulDetailOpen}>Detayı aç  →</Text></View></Pressable><View style={styles.izmirDistrictPlaceActions}><Pressable onPress={()=>openDirections(place.mapQuery)} style={styles.izmirDistrictSecondaryButton}><Text style={styles.izmirDistrictSecondaryText}>📍 Yol tarifi</Text></Pressable><Pressable onPress={()=>onTogglePlan(place.id)} style={[styles.izmirDistrictSecondaryButton,plannedPlaceIds.includes(place.id)&&styles.izmirDistrictSecondaryButtonActive]}><Text style={[styles.izmirDistrictSecondaryText,plannedPlaceIds.includes(place.id)&&styles.izmirDistrictSecondaryTextActive]}>{plannedPlaceIds.includes(place.id)?'✓ Planımda':'+ Planıma ekle'}</Text></Pressable></View></View>)}</View>:<View style={styles.izmirDistrictEmpty}><Text style={styles.izmirDistrictEmptyTitle}>Doğrulanmış ayrıntılı kart hazırlanıyor</Text><Text style={styles.izmirDistrictEmptyCopy}>Bu ilçenin doğrulanmış “görmeden dönme” noktalarını aşağıdaki harita keşfiyle inceleyebilirsiniz.</Text></View>;
  const routeStops=[...district.highlights,...beaches.slice(0,2).map(beach=>beach.name)].slice(0,5);
  const sectionTitle=tabs.find(tab=>tab.id===section)?.label??'Genel Bakış';

  return <Modal visible animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}><SafeAreaView style={styles.izmirDistrictGuide}>
    <View style={[styles.izmirDistrictHero,width>=900&&styles.izmirDistrictHeroDesktop]}>{heroPlace?<Image source={heroPlace.image} style={styles.izmirDistrictHeroImage}/>:<View style={styles.izmirDistrictHeroFallback}/>}<View style={styles.izmirDistrictHeroShade}/><View style={styles.izmirDistrictHeroTop}><View><Text style={styles.izmirDistrictHeroKicker}>İZMİR · İLÇE GEZİ REHBERİ</Text><Text style={styles.izmirDistrictHeroTitle}>{district.name}</Text></View><Pressable onPress={onClose} style={styles.izmirDistrictClose}><Text style={styles.districtCloseText}>×</Text></Pressable></View><View style={styles.izmirDistrictHeroCopy}><Text style={styles.izmirDistrictHeroTagline}>{profile?.tagline??district.signature}</Text><Text numberOfLines={2} style={styles.izmirDistrictHeroSignature}>{district.signature}</Text></View></View>
    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.izmirDistrictTabsScroll} contentContainerStyle={styles.izmirDistrictTabs}>{tabs.map(tab=><Pressable key={tab.id} onPress={()=>setSection(tab.id)} style={[styles.izmirDistrictTab,section===tab.id&&styles.izmirDistrictTabActive]}><Text style={styles.izmirDistrictTabIcon}>{tab.icon}</Text><Text numberOfLines={1} style={[styles.izmirDistrictTabText,section===tab.id&&styles.izmirDistrictTabTextActive]}>{tab.label}</Text></Pressable>)}</ScrollView>
    <ScrollView contentContainerStyle={[styles.izmirDistrictGuideBody,width>=900&&styles.izmirDistrictGuideBodyDesktop]}>
      <View style={styles.izmirDistrictBreadcrumb}><Pressable onPress={onClose}><Text style={styles.izmirDistrictBreadcrumbBack}>← İzmir İlçeleri</Text></Pressable><Text style={styles.izmirDistrictBreadcrumbText}>İzmir › {district.name} › {sectionTitle}</Text></View>
      {section==='overview'&&<><View style={styles.izmirDistrictIdentity}><View><Text style={styles.izmirDistrictSectionEyebrow}>GENEL BAKIŞ</Text><Text style={styles.izmirDistrictSectionTitle}>{district.name}</Text><Text style={styles.izmirDistrictSectionCopy}>📍 İzmir · {district.signature}</Text></View><View style={styles.izmirDistrictThemeWrap}>{themes.map(theme=><View key={theme} style={styles.izmirDistrictThemeChip}><Text style={styles.izmirDistrictThemeText}>{districtThemeIcon(theme)} {theme}</Text></View>)}</View></View><View style={styles.izmirDistrictGuidePanel}><Text style={styles.izmirDistrictPanelTitle}>Neden gitmeli?</Text><Text style={styles.izmirDistrictPanelCopy}>{profile?.whyVisit??`${district.name}; ${district.signature.toLocaleLowerCase('tr-TR')} ile İzmir'in farklı yüzlerini keşfetmek isteyenler için anlamlı bir rotadır.`}</Text></View><View style={styles.izmirDistrictGuidePanel}><Text style={styles.izmirDistrictPanelEyebrow}>⭐ BU İLÇEDE KAÇIRMA</Text>{district.highlights.map((item,index)=>{const matched=places.find(place=>place.name.toLocaleLowerCase('tr-TR').includes(item.toLocaleLowerCase('tr-TR'))||item.toLocaleLowerCase('tr-TR').includes((place.name.split(' ve ')[0]??place.name).toLocaleLowerCase('tr-TR')));return <Pressable key={item} onPress={()=>matched?setSelectedPlace(matched):openSearch(item)} style={styles.districtListItem}><Text style={styles.districtListIndex}>{String(index+1).padStart(2,'0')}</Text><Text style={styles.districtListText}>{item}</Text><Text style={styles.districtListArrow}>{matched?'→':'↗'}</Text></Pressable>})}</View>{places.length>0&&<><Text style={styles.izmirDistrictContentTitle}>Öne çıkan ayrıntılı rehberler</Text>{renderPlaceCards(places.slice(0,4))}</>}</>}
      {section==='places'&&<><Text style={styles.izmirDistrictSectionEyebrow}>📍 GEZİLECEK YERLER</Text><Text style={styles.izmirDistrictContentTitle}>{district.name} durakları</Text>{renderPlaceCards(places)}<View style={styles.izmirDistrictGuidePanel}><Text style={styles.izmirDistrictPanelEyebrow}>DİĞER DOĞRULANMIŞ ÖNERİLER</Text>{district.highlights.filter(item=>!places.some(place=>place.name.includes(item))).map((item,index)=><Pressable key={item} onPress={()=>openSearch(item)} style={styles.districtListItem}><Text style={styles.districtListIndex}>{String(index+1).padStart(2,'0')}</Text><Text style={styles.districtListText}>{item}</Text><Text style={styles.districtListArrow}>↗</Text></Pressable>)}</View></>}
      {section==='beaches'&&<><Text style={styles.izmirDistrictSectionEyebrow}>🏖️ PLAJLAR & KOYLAR</Text><Text style={styles.izmirDistrictContentTitle}>{district.name} kıyı rehberi</Text><Text style={styles.izmirDistrictSectionIntro}>İzmir ana plaj veri kümesinden yalnızca {district.name} kayıtları gösteriliyor; doğrulanmamış özellikler “yok” sayılmaz.</Text><View style={styles.izmirDistrictPlaceGrid}>{beaches.map(beach=><IzmirBeachCard key={beach.id} beach={beach} favorite={favorites.includes(beach.id)} planned={plannedPlaceIds.includes(beach.id)} onFavorite={onFavorite} onPlan={onTogglePlan} onOpen={()=>setSelectedBeach(beach)}/>)}</View></>}
      {section==='food'&&<><Text style={styles.izmirDistrictSectionEyebrow}>🍴 NE YENİR?</Text><Text style={styles.izmirDistrictContentTitle}>{district.name} lezzetleri</Text><View style={styles.izmirDistrictGuidePanel}><View style={styles.flavorWrap}>{district.flavors.map(item=><Pressable key={item} onPress={()=>openSearch(`${item} nerede yenir`)} style={styles.flavorChip}><Text style={styles.flavorText}>{item} ↗</Text></Pressable>)}</View></View>{foods.map(item=><Pressable key={item.dish} onPress={()=>Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.mapQuery)}`)} style={styles.izmirDistrictGuidePanel}><Text style={styles.izmirDistrictPanelTitle}>{item.dish}</Text><Text style={styles.izmirDistrictPanelCopy}>{item.note}</Text><Text style={styles.izmirDistrictExploreLink}>{item.area} · Haritada ara ↗</Text></Pressable>)}</>}
      {section==='venues'&&<><Text style={styles.izmirDistrictSectionEyebrow}>☕ KAFE & RESTORAN</Text><Text style={styles.izmirDistrictContentTitle}>Bölge bölge yeme & içme</Text>{venues.length?venues.map(item=><View key={item.area} style={styles.izmirDistrictGuidePanel}><Text style={styles.izmirDistrictPanelTitle}>{item.area}</Text><Text style={styles.izmirDistrictPanelCopy}>{item.character}</Text><View style={styles.izmirDistrictInlineActions}><Pressable onPress={()=>Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.cafeQuery)}`)} style={styles.izmirDistrictInlineButton}><Text style={styles.izmirDistrictInlineText}>Kafeleri göster ↗</Text></Pressable><Pressable onPress={()=>Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.restaurantQuery)}`)} style={styles.izmirDistrictInlineButton}><Text style={styles.izmirDistrictInlineText}>Restoranları göster ↗</Text></Pressable></View></View>):<View style={styles.izmirDistrictGuidePanel}><Text style={styles.izmirDistrictPanelTitle}>{district.name} merkezinde güncel seçenekler</Text><Text style={styles.izmirDistrictPanelCopy}>İşletmeler değişebildiği için sabit liste yerine güncel harita sonuçları kullanılır.</Text><View style={styles.izmirDistrictInlineActions}><Pressable onPress={()=>openSearch('kafeler')} style={styles.izmirDistrictInlineButton}><Text style={styles.izmirDistrictInlineText}>Kafeler ↗</Text></Pressable><Pressable onPress={()=>openSearch('restoranlar')} style={styles.izmirDistrictInlineButton}><Text style={styles.izmirDistrictInlineText}>Restoranlar ↗</Text></Pressable></View></View>}</>}
      {section==='nightlife'&&<><Text style={styles.izmirDistrictSectionEyebrow}>🌙 GECE HAYATI</Text><Text style={styles.izmirDistrictContentTitle}>{district.name} akşamları</Text>{nightlife.map(item=><Pressable key={item.area} onPress={()=>Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.mapQuery)}`)} style={styles.izmirDistrictGuidePanel}><Text style={styles.izmirDistrictPanelTitle}>{item.area}</Text><Text style={styles.izmirDistrictPanelCopy}>{item.character}</Text><Text style={styles.izmirDistrictExploreLink}>Bölgede ara ↗</Text></Pressable>)}</>}
      {section==='shopping'&&<><Text style={styles.izmirDistrictSectionEyebrow}>🛍️ ALIŞVERİŞ</Text><Text style={styles.izmirDistrictContentTitle}>Çarşılar & yerel üretim</Text>{shopping.map(item=><Pressable key={item.area} onPress={()=>Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.mapQuery)}`)} style={styles.izmirDistrictGuidePanel}><Text style={styles.izmirDistrictPanelTitle}>{item.area}</Text><Text style={styles.izmirDistrictPanelCopy}>{item.character}</Text><Text style={styles.izmirDistrictExploreLink}>Haritada keşfet ↗</Text></Pressable>)}</>}
      {section==='nature'&&<><Text style={styles.izmirDistrictSectionEyebrow}>🌿 DOĞA</Text><Text style={styles.izmirDistrictContentTitle}>Açık hava durakları</Text>{naturePlaces.length?renderPlaceCards(naturePlaces):<View style={styles.izmirDistrictGuidePanel}>{district.highlights.map((item,index)=><Pressable key={item} onPress={()=>openSearch(`${item} doğa`)} style={styles.districtListItem}><Text style={styles.districtListIndex}>{String(index+1).padStart(2,'0')}</Text><Text style={styles.districtListText}>{item}</Text><Text style={styles.districtListArrow}>↗</Text></Pressable>)}</View>}</>}
      {section==='family'&&<><Text style={styles.izmirDistrictSectionEyebrow}>👨‍👩‍👧 AİLECE</Text><Text style={styles.izmirDistrictContentTitle}>Kolay ve birlikte keşif</Text>{familyRoutes.map(item=><Pressable key={item.area} onPress={()=>Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.mapQuery)}`)} style={styles.izmirDistrictGuidePanel}><Text style={styles.izmirDistrictPanelTitle}>{item.area}</Text><Text style={styles.izmirDistrictPanelCopy}>{item.character}</Text><Text style={styles.izmirDistrictExploreLink}>Haritada keşfet ↗</Text></Pressable>)}{familyPlaces.length>0&&renderPlaceCards(familyPlaces)}{!familyRoutes.length&&!familyPlaces.length&&<View style={styles.izmirDistrictGuidePanel}><Text style={styles.izmirDistrictPanelCopy}>Kısa yürüyüş, park ve aileye uygun güncel seçenekleri ilçe çevresinde haritada karşılaştırın.</Text><Pressable onPress={()=>openSearch('ailece gezilecek yerler')} style={styles.izmirDistrictInlineButton}><Text style={styles.izmirDistrictInlineText}>Ailece yerleri ara ↗</Text></Pressable></View>}</>}
      {section==='photos'&&<><Text style={styles.izmirDistrictSectionEyebrow}>📸 FOTOĞRAF NOKTALARI</Text><Text style={styles.izmirDistrictContentTitle}>{district.name} kadrajları</Text>{places.length?renderPlaceCards(places):<View style={styles.izmirDistrictGuidePanel}>{district.highlights.map((item,index)=><Pressable key={item} onPress={()=>openSearch(item)} style={styles.districtListItem}><Text style={styles.districtListIndex}>{String(index+1).padStart(2,'0')}</Text><Text style={styles.districtListText}>{item}</Text><Text style={styles.districtListArrow}>↗</Text></Pressable>)}</View>}</>}
      {section==='routes'&&<><Text style={styles.izmirDistrictSectionEyebrow}>🗺️ HAZIR ROTALAR</Text><Text style={styles.izmirDistrictContentTitle}>Bir günde {district.name}</Text><View style={styles.izmirDistrictRouteCard}>{routeStops.map((stop,index)=><View key={stop} style={styles.izmirDistrictRouteStop}><Text style={styles.izmirDistrictRouteTime}>{index===0?'SABAH':index===routeStops.length-1?'AKŞAM':index===1?'ÖĞLE':'ÖĞLEDEN SONRA'}</Text><Text style={styles.izmirDistrictRouteName}>{stop}</Text></View>)}<Pressable onPress={()=>openDistrictRoute(routeStops)} style={styles.districtMapButton}><Text style={styles.districtMapText}>Rotayı haritada başlat →</Text></Pressable></View>{beaches.length>=3&&<View style={styles.izmirDistrictGuidePanel}><Text style={styles.izmirDistrictPanelTitle}>🏖️ {district.name} plaj rotası</Text><Text style={styles.izmirDistrictPanelCopy}>{beaches.slice(0,3).map(item=>item.name).join(' → ')}</Text><Pressable onPress={()=>openDistrictRoute(beaches.slice(0,3).map(item=>item.name))} style={styles.izmirDistrictInlineButton}><Text style={styles.izmirDistrictInlineText}>Rotayı aç →</Text></Pressable></View>}</>}
      {section==='practical'&&<><Text style={styles.izmirDistrictSectionEyebrow}>ℹ️ PRATİK BİLGİLER</Text><Text style={styles.izmirDistrictContentTitle}>{district.name} çevresinde ihtiyaçlar</Text><View style={styles.izmirDistrictGuidePanel}><View style={styles.districtServiceGrid}>{services.map(([icon,label,q])=><Pressable key={label} onPress={()=>openSearch(q)} style={styles.districtServiceButton}><Text style={styles.districtServiceIcon}>{icon}</Text><Text style={styles.districtServiceText}>{label}</Text></Pressable>)}</View></View><Pressable onPress={()=>openSearch('gezilecek yerler')} style={styles.districtMapButton}><Text style={styles.districtMapText}>🗺️ Haritada {district.name} →</Text></Pressable><Text style={styles.izmirDistrictPracticalNote}>İşletme saatleri, ulaşım koşulları ve hizmet bilgileri değişebilir; ziyaret öncesinde resmî kaynakları ve güncel harita sonuçlarını kontrol edin.</Text></>}
      <Pressable onPress={()=>onTogglePlan(`izmir-ilce-${district.name}`)} style={[styles.izmirDistrictPlanButton,planned&&styles.izmirDistrictPlanButtonActive]}><Text style={[styles.izmirDistrictPlanText,planned&&styles.izmirDistrictPlanTextActive]}>{planned?'✓ İlçe gezi planımda':'+ İlçeyi gezi planıma ekle'}</Text></Pressable>
    </ScrollView>
    <IstanbulPlaceModal place={selectedPlace} favorite={selectedPlace?favorites.includes(selectedPlace.id):false} planned={selectedPlace?plannedPlaceIds.includes(selectedPlace.id):false} onFavorite={onFavorite} onTogglePlan={onTogglePlan} onClose={()=>setSelectedPlace(null)}/><IzmirBeachModal beach={selectedBeach} favorite={selectedBeach?favorites.includes(selectedBeach.id):false} planned={selectedBeach?plannedPlaceIds.includes(selectedBeach.id):false} onFavorite={onFavorite} onPlan={onTogglePlan} onClose={()=>setSelectedBeach(null)}/>
  </SafeAreaView></Modal>;
}

function districtThemeIcon(theme:IzmirDistrictTheme){return theme==='Deniz'?'🌊':theme==='Tarih'?'🏛️':theme==='Doğa'?'🌿':theme==='Gastronomi'?'🍴':theme==='Aile'?'👨‍👩‍👧':'🌙';}
...
function BottomTabs({ tab, setTab, favoriteCount, planCount, izmirThemed = false }: { tab: Tab; setTab: (tab: Tab) => void; favoriteCount: number; planCount: number; izmirThemed?: boolean }) {
  const { width } = useWindowDimensions();
  if (izmirThemed && width >= 1024) return null;
  ...
  return <View style={[styles.tabBar,izmirThemed&&styles.izmirTabBar,izmirThemed&&width>=768&&styles.izmirTabBarTablet]}>{items.map(item => { const badge = item.id === 'favorites' ? favoriteCount : item.id === 'plan' ? planCount : 0; const active=tab===item.id; return <Pressable key={item.id} onPress={() => setTab(item.id)} style={styles.tabItem}>{active&&izmirThemed&&<View style={styles.izmirTabIndicator}/>}<View><Text style={[styles.tabIcon,izmirThemed&&styles.izmirTabInactive,active&&styles.tabActive]}>{item.icon}</Text>{badge > 0 && <View style={[styles.tabBadge,izmirThemed&&styles.izmirTabBadge]}><Text style={styles.tabBadgeText}>{badge}</Text></View>}</View><Text style={[styles.tabLabel,izmirThemed&&styles.izmirTabInactive,active&&styles.tabActive]}>{item.label}</Text></Pressable>; })}</View>;
}
...
const styles = StyleSheet.create({
  ...
  izmirDistrictGuide: { flex: 1, backgroundColor: izmirTheme.cream },
  izmirDistrictHero: { height: 280, overflow: 'hidden', backgroundColor: izmirTheme.forestDark },
  izmirDistrictHeroDesktop: { height: 360 },
  izmirDistrictHeroImage: { position: 'absolute', inset: 0, width: '100%', height: '100%' },
  izmirDistrictHeroFallback: { position: 'absolute', inset: 0, backgroundColor: izmirTheme.forest },
  izmirDistrictHeroShade: { position: 'absolute', inset: 0, ...(Platform.OS==='web'?({backgroundImage:'linear-gradient(90deg, rgba(16,47,40,.86) 0%, rgba(16,47,40,.42) 55%, rgba(16,47,40,.10) 100%)'} as object):{backgroundColor:'rgba(16,47,40,.46)'}) },
  izmirDistrictHeroTop: { padding: 22, paddingTop: Platform.OS==='android'?(NativeStatusBar.currentHeight??24)+16:24, flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' },
  izmirDistrictHeroKicker: { color: izmirTheme.goldSoft, fontSize: 9, fontWeight: '900', letterSpacing: 1.4 },
  izmirDistrictHeroTitle: { marginTop: 8, color: izmirTheme.white, fontFamily: Platform.select({ios:'Georgia',android:'serif'}), fontSize: 44, fontWeight: '700', letterSpacing: -1.2 },
  izmirDistrictClose: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,255,255,.18)', borderWidth: 1, borderColor: 'rgba(255,255,255,.28)' },
  izmirDistrictHeroCopy: { position: 'absolute', right: 24, bottom: 24, left: 24, maxWidth: 760 },
  izmirDistrictHeroTagline: { color: izmirTheme.white, fontFamily: Platform.select({ios:'Georgia',android:'serif'}), fontSize: 22, lineHeight: 28, fontWeight: '700' },
  izmirDistrictHeroSignature: { marginTop: 7, color: izmirTheme.creamSoft, fontSize: 13, lineHeight: 19 },
  izmirDistrictTabsScroll: { flexGrow: 0, borderBottomWidth: 1, borderBottomColor: izmirTheme.line, backgroundColor: izmirTheme.paper },
  izmirDistrictTabs: { minHeight: 64, paddingHorizontal: 14, paddingVertical: 8, alignItems: 'center', gap: 8 },
  izmirDistrictTab: { minHeight: 46, paddingHorizontal: 15, flexDirection: 'row', alignItems: 'center', gap: 7, borderRadius: 16, borderWidth: 1, borderColor: izmirTheme.lineGreen, backgroundColor: izmirTheme.paper },
  izmirDistrictTabActive: { borderColor: izmirTheme.forest, backgroundColor: izmirTheme.forest },
  izmirDistrictTabIcon: { fontSize: 14 },
  izmirDistrictTabText: { color: izmirTheme.forest, fontSize: 11, fontWeight: '900' },
  izmirDistrictTabTextActive: { color: izmirTheme.white },
  izmirDistrictGuideBody: { width: '100%', padding: 16, paddingBottom: 80, gap: 16 },
  izmirDistrictGuideBodyDesktop: { maxWidth: 1180, alignSelf: 'center', paddingHorizontal: 28, paddingTop: 24 },
  izmirDistrictBreadcrumb: { minHeight: 42, flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 14 },
  izmirDistrictBreadcrumbBack: { color: izmirTheme.forest, fontSize: 12, fontWeight: '900' },
  izmirDistrictBreadcrumbText: { color: izmirTheme.muted, fontSize: 11, fontWeight: '700' },
  izmirDistrictIdentity: { padding: 22, borderRadius: 22, borderWidth: 1, borderColor: izmirTheme.line, backgroundColor: izmirTheme.paper, gap: 18 },
  izmirDistrictSectionEyebrow: { color: izmirTheme.moss, fontSize: 10, fontWeight: '900', letterSpacing: 1.3 },
  izmirDistrictSectionTitle: { marginTop: 7, color: izmirTheme.forest, fontFamily: Platform.select({ios:'Georgia',android:'serif'}), fontSize: 34, fontWeight: '700' },
  izmirDistrictSectionCopy: { marginTop: 6, color: izmirTheme.muted, fontSize: 13, lineHeight: 20 },
  izmirDistrictSectionIntro: { marginTop: -8, marginBottom: 4, color: izmirTheme.muted, fontSize: 13, lineHeight: 20 },
  izmirDistrictThemeWrap: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  izmirDistrictThemeChip: { paddingHorizontal: 12, paddingVertical: 9, borderRadius: 16, backgroundColor: izmirTheme.seaSoft },
  izmirDistrictThemeText: { color: izmirTheme.forest, fontSize: 11, fontWeight: '800' },
  izmirDistrictGuidePanel: { padding: 21, borderRadius: 22, borderWidth: 1, borderColor: izmirTheme.line, backgroundColor: izmirTheme.paper },
  izmirDistrictPanelEyebrow: { marginBottom: 10, color: izmirTheme.gold, fontSize: 10, fontWeight: '900', letterSpacing: 1.2 },
  izmirDistrictPanelTitle: { color: izmirTheme.forest, fontFamily: Platform.select({ios:'Georgia',android:'serif'}), fontSize: 21, fontWeight: '700' },
  izmirDistrictPanelCopy: { marginTop: 9, color: izmirTheme.muted, fontSize: 14, lineHeight: 22 },
  izmirDistrictContentTitle: { color: izmirTheme.forest, fontFamily: Platform.select({ios:'Georgia',android:'serif'}), fontSize: 28, fontWeight: '700' },
  izmirDistrictPlaceGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 14 },
  izmirDistrictPlaceCard: { width: '100%', maxWidth: 540 },
  izmirDistrictPlaceActions: { paddingHorizontal: 18, paddingBottom: 18, flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  izmirDistrictSecondaryButton: { minHeight: 44, paddingHorizontal: 13, flexGrow: 1, alignItems: 'center', justifyContent: 'center', borderRadius: 14, borderWidth: 1, borderColor: izmirTheme.lineGreen, backgroundColor: izmirTheme.paper },
  izmirDistrictSecondaryButtonActive: { borderColor: izmirTheme.forest, backgroundColor: izmirTheme.forest },
  izmirDistrictSecondaryText: { color: izmirTheme.forest, fontSize: 10, fontWeight: '900' },
  izmirDistrictSecondaryTextActive: { color: izmirTheme.white },
  izmirDistrictEmpty: { padding: 22, borderRadius: 20, borderWidth: 1, borderColor: izmirTheme.line, backgroundColor: izmirTheme.paper },
  izmirDistrictEmptyTitle: { color: izmirTheme.forest, fontSize: 16, fontWeight: '900' },
  izmirDistrictEmptyCopy: { marginTop: 7, color: izmirTheme.muted, fontSize: 13, lineHeight: 20 },
  izmirDistrictExploreLink: { marginTop: 14, color: izmirTheme.moss, fontSize: 11, fontWeight: '900' },
  izmirDistrictInlineActions: { marginTop: 15, flexDirection: 'row', flexWrap: 'wrap', gap: 9 },
  izmirDistrictInlineButton: { minHeight: 44, paddingHorizontal: 15, alignItems: 'center', justifyContent: 'center', borderRadius: 14, backgroundColor: izmirTheme.seaSoft },
  izmirDistrictInlineText: { color: izmirTheme.forest, fontSize: 11, fontWeight: '900' },
  izmirDistrictRouteCard: { padding: 22, borderRadius: 22, backgroundColor: izmirTheme.forest },
  izmirDistrictRouteStop: { minHeight: 58, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,.14)' },
  izmirDistrictRouteTime: { color: izmirTheme.goldSoft, fontSize: 9, fontWeight: '900', letterSpacing: 1.1 },
  izmirDistrictRouteName: { marginTop: 5, color: izmirTheme.white, fontSize: 16, fontWeight: '800' },
  izmirDistrictPracticalNote: { color: izmirTheme.muted, fontSize: 11, lineHeight: 18, textAlign: 'center' },
  izmirDistrictPlanButton: { minHeight: 54, alignItems: 'center', justifyContent: 'center', borderRadius: 18, borderWidth: 1, borderColor: izmirTheme.forest, backgroundColor: izmirTheme.paper },
  izmirDistrictPlanButtonActive: { backgroundColor: izmirTheme.forest },
  izmirDistrictPlanText: { color: izmirTheme.forest, fontSize: 12, fontWeight: '900' },
  izmirDistrictPlanTextActive: { color: izmirTheme.white },
  ...
  izmirPage: { backgroundColor: izmirTheme.cream },
  izmirContent: { paddingBottom: 118, backgroundColor: izmirTheme.cream },
  izmirContentTablet: { paddingBottom: 105 },
  izmirContentDesktop: { paddingBottom: 64 },
  izmirHeader: { width: '100%', maxWidth: 1380, minHeight: 58, alignSelf: 'center', paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: izmirTheme.line, backgroundColor: izmirTheme.paper },
  izmirHeaderDesktop: { minHeight: 64, paddingHorizontal: 22 },
  izmirBrand: { minWidth: 0, flexDirection: 'row', alignItems: 'center', gap: 9 },
  izmirBrandMark: { color: izmirTheme.gold, fontSize: 25, lineHeight: 28, fontWeight: '900' },
  izmirBrandText: { flexShrink: 1, color: izmirTheme.forest, fontFamily: Platform.select({ ios: 'Georgia', android: 'serif' }), fontSize: 16, fontWeight: '700' },
  izmirHeaderActions: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  izmirHeaderLink: { minHeight: 42, paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center', gap: 6, borderRadius: 13 },
  izmirHeaderLinkIcon: { color: izmirTheme.gold, fontSize: 20, fontWeight: '900' },
  izmirHeaderLinkText: { color: izmirTheme.forest, fontSize: 11, fontWeight: '900' },
  izmirHeaderMenu: { width: 42, height: 42, alignItems: 'center', justifyContent: 'center', borderRadius: 14, borderWidth: 1, borderColor: izmirTheme.forest, backgroundColor: izmirTheme.forest },
  izmirHeaderMenuText: { color: izmirTheme.paper, fontSize: 20, fontWeight: '900' },
  izmirHeaderHeart: { color: izmirTheme.paper, fontSize: 25, fontWeight: '900' },
  izmirPremiumHero: { width: 'auto', maxWidth: 1380, alignSelf: 'stretch', borderRadius: 0 },
  izmirPremiumHeroDesktop: { height: 400 },
  izmirPremiumHeroMobile: { height: 280 },
  izmirPremiumHeroShade: { backgroundColor: 'rgba(16,47,40,.02)' },
  izmirHeroLeftGradient: { position: 'absolute', inset: 0, ...(Platform.OS === 'web' ? ({ backgroundImage: 'linear-gradient(90deg, rgba(16,47,40,.72) 0%, rgba(16,47,40,.25) 48%, rgba(16,47,40,.02) 78%)' } as object) : { backgroundColor: 'rgba(16,47,40,.25)' }) },
  izmirPremiumHeroBody: { width: '100%', maxWidth: 1280, alignSelf: 'center', alignItems: 'flex-start', justifyContent: 'center', paddingHorizontal: 28, paddingBottom: 0 },
  izmirPremiumHeroTitle: { fontSize: 72, letterSpacing: 2 },
  izmirHeroSubtitle: { marginTop: 4, color: '#D9AE63', fontFamily: Platform.select({ ios: 'Georgia', android: 'serif' }), fontSize: 20, fontWeight: '600' },
  izmirPremiumHeroCopy: { maxWidth: 430, color: izmirTheme.creamSoft, textAlign: 'left' },
  izmirGuideNav: { width: '96%', maxWidth: 1280, alignSelf: 'center', marginTop: -24, paddingHorizontal: 8, borderWidth: 1, borderColor: izmirTheme.line, borderRadius: 19, backgroundColor: izmirTheme.paper, shadowColor: izmirTheme.forest, shadowOffset: { width: 0, height: 8 }, shadowOpacity: .10, shadowRadius: 18, elevation: 5, zIndex: 4 },
  izmirGuideNavDesktop: { marginTop: -42, paddingHorizontal: 8 },
  izmirGuideNavRail: { gap: 7, paddingVertical: 8, paddingRight: 10 },
  izmirGuideNavRailDesktop: { flexGrow: 1, paddingRight: 0 },
  izmirGuideNavItem: { minWidth: 138, minHeight: 58, paddingHorizontal: 14, alignItems: 'center', justifyContent: 'center', gap: 4, borderRadius: 14, borderWidth: 1, borderColor: 'transparent', backgroundColor: izmirTheme.paper },
  izmirGuideNavItemDesktop: { flex: 1, minWidth: 0, minHeight: 76, paddingHorizontal: 10 },
  izmirGuideNavItemActive: { borderColor: izmirTheme.forest, backgroundColor: izmirTheme.forest },
  izmirGuideNavText: { color: izmirTheme.forest, fontSize: 11, fontWeight: '900' },
  izmirGuideNavTextDesktop: { color: izmirTheme.forest, fontSize: 11, letterSpacing: .2 },
  izmirGuideNavTextActive: { color: izmirTheme.white },
  izmirGuideNavIcon: { color: izmirTheme.moss, fontSize: 20, lineHeight: 22, fontWeight: '900' },
  ...
  izmirBreadcrumbBack: { minWidth: 140, height: 44, paddingHorizontal: 18, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', borderRadius: 20, borderWidth: 1, borderColor: izmirTheme.lineGreen, backgroundColor: izmirTheme.paper },
  izmirBreadcrumbBackText: { color: izmirTheme.forest, fontSize: 11, lineHeight: 16, fontWeight: '900' },
  izmirBreadcrumbBackGap: { letterSpacing: 3 },
  izmirBreadcrumbCurrent: { flexGrow: 1, flexShrink: 0, fontSize: 11, fontWeight: '800' },
  izmirBreadcrumbCity: { color: izmirTheme.muted },
  izmirBreadcrumbSeparator: { color: izmirTheme.gold },
  izmirBreadcrumbActive: { color: izmirTheme.forest },
  izmirOverviewIntro: { width: '100%', maxWidth: 860, alignSelf: 'center', marginTop: 50, marginBottom: 8, alignItems: 'center' },
  izmirOverviewTitleRow: { width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10 },
  izmirOverviewLine: { width: 34, height: 1, backgroundColor: izmirTheme.gold },
  izmirOverviewDot: { width: 5, height: 5, borderRadius: 3, backgroundColor: izmirTheme.gold },
  izmirOverviewTitleDesktop: { fontSize: 38, lineHeight: 45 },
  izmirHeadingText: { color: izmirTheme.forest },
  izmirOverviewCopy: { marginTop: 8, color: izmirTheme.muted, fontSize: 15, lineHeight: 23, textAlign: 'center' },
  ...
  izmirDashboardPanel: { flexGrow: 1, flexBasis: 280, padding: 22, borderRadius: 22, borderWidth: 1, borderColor: izmirTheme.line, backgroundColor: izmirTheme.paper, shadowColor: izmirTheme.forest, shadowOffset: { width: 0, height: 8 }, shadowOpacity: .10, shadowRadius: 18, elevation: 2 },
  izmirDashboardFacts: { flexGrow: 0, flexShrink: 1, flexBasis: '32%' },
  izmirDashboardFeatured: { flexGrow: 1, flexShrink: 1, flexBasis: '68%' },
  izmirDashboardTabletHalf: { flexBasis: '47%' },
  izmirDashboardTabletFull: { flexBasis: '100%' },
  izmirFactsPanel: { backgroundColor: izmirTheme.paper },
  izmirFeaturedPanel: { borderColor: izmirTheme.forest, backgroundColor: izmirTheme.forest },
  izmirPopularPanel: { backgroundColor: '#EEF3EF' },
  izmirPanelEyebrow: { marginBottom: 16, color: izmirTheme.forest, fontSize: 10, fontWeight: '900', letterSpacing: 1.3 },
  izmirFeaturedEyebrow: { marginBottom: 16, color: izmirTheme.white, fontSize: 10, fontWeight: '900', letterSpacing: 1.3 },
  izmirFactsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 9 },
  izmirFact: { width: '47%', minHeight: 86, padding: 11, flexDirection: 'row', alignItems: 'center', gap: 9, borderRadius: 15, borderWidth: 1, borderColor: izmirTheme.line },
  izmirFactIcon: { width: 28, color: izmirTheme.moss, fontSize: 20, fontWeight: '900', textAlign: 'center' },
  izmirFactBody: { flex: 1, minWidth: 0 },
  izmirFactValue: { color: izmirTheme.forest, fontSize: 18, lineHeight: 21, fontWeight: '900' },
  izmirFactLabel: { marginTop: 4, color: izmirTheme.muted, fontSize: 9, fontWeight: '900', textTransform: 'uppercase' },
  izmirFactNote: { marginTop: 3, color: palette.muted, fontSize: 8, fontWeight: '700' },
  izmirFactsFootnote: { marginTop: 12, color: izmirTheme.muted, fontSize: 8, lineHeight: 13 },
  ...
  izmirFeaturedRail: { gap: 12, paddingRight: 8 },
  izmirFeaturedCard: { width: '48%', minHeight: 150, overflow: 'hidden', borderRadius: 16, backgroundColor: izmirTheme.forestDark },
  izmirFeaturedCardMobile: { width: 188, minHeight: 158 },
  izmirFeaturedImage: { position: 'absolute', inset: 0, width: '100%', height: '100%' },
  izmirFeaturedShade: { position: 'absolute', inset: 0, ...(Platform.OS === 'web' ? ({ backgroundImage: 'linear-gradient(180deg, rgba(16,47,40,0) 28%, rgba(16,47,40,.82) 100%)' } as object) : { backgroundColor: 'rgba(16,47,40,.38)' }) },
  izmirFeaturedBody: { flex: 1, padding: 13, justifyContent: 'flex-end' },
  izmirFeaturedDistrict: { color: izmirTheme.goldSoft, fontSize: 9, fontWeight: '900', letterSpacing: 1 },
  izmirFeaturedName: { marginTop: 4, color: izmirTheme.white, fontFamily: Platform.select({ ios: 'Georgia', android: 'serif' }), fontSize: 17, lineHeight: 21, fontWeight: '600' },
  izmirFeaturedAll: { minHeight: 42, marginTop: 10, paddingHorizontal: 14, alignSelf: 'flex-end', alignItems: 'center', justifyContent: 'center', borderRadius: 10, backgroundColor: izmirTheme.paper },
  izmirFeaturedAllText: { color: izmirTheme.forest, fontSize: 10, fontWeight: '900' },
  izmirGuideHeading: { marginTop: 54, marginBottom: 22, alignItems: 'center' },
  ...
  izmirIntentGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 14 },
  izmirIntentGridDesktop: { flexWrap: 'nowrap', gap: 16 },
  izmirIntentCard: { flexGrow: 1, flexBasis: 260, minHeight: 112, padding: 18, flexDirection: 'row', alignItems: 'center', gap: 12, borderRadius: 19, borderWidth: 1, borderColor: izmirTheme.lineGreen, shadowColor: izmirTheme.forest, shadowOffset: { width: 0, height: 6 }, shadowOpacity: .10, shadowRadius: 12, elevation: 2 },
  izmirIntentIcon: { width: 42, height: 42, alignItems: 'center', justifyContent: 'center', borderRadius: 21, backgroundColor: 'rgba(255,255,255,.18)' },
  izmirIntentIconText: { color: izmirTheme.forest, fontSize: 24, fontWeight: '900' },
  izmirIntentBody: { flex: 1, minWidth: 0 },
  izmirIntentTitle: { color: izmirTheme.ink, fontFamily: Platform.select({ ios: 'Georgia', android: 'serif' }), fontSize: 17, fontWeight: '700' },
  izmirIntentCopy: { marginTop: 5, color: izmirTheme.muted, fontSize: 10, lineHeight: 15 },
  izmirIntentArrow: { color: izmirTheme.forest, fontSize: 21, fontWeight: '900' },
  izmirIntentHistory: { backgroundColor: izmirTheme.forest }, izmirIntentHistoryText: { color: izmirTheme.goldSoft }, izmirIntentHistoryCopy: { color: '#E8EEE9' },
  izmirIntentSea: { backgroundColor: izmirTheme.seaSoft }, izmirIntentSeaText: { color: izmirTheme.forest }, izmirIntentSeaCopy: { color: '#557068' },
  izmirIntentFamily: { backgroundColor: izmirTheme.sandSoft }, izmirIntentFamilyText: { color: izmirTheme.forest }, izmirIntentFamilyCopy: { color: '#6F685A' },
  izmirIntentEvening: { backgroundColor: izmirTheme.moss }, izmirIntentEveningText: { color: izmirTheme.goldSoft }, izmirIntentEveningCopy: { color: '#EDF3EF' },
  izmirDistrictSection: { marginTop: 56, paddingTop: 2 },
  izmirDistrictIntro: { maxWidth: 720, marginTop: 8, color: izmirTheme.muted, fontSize: 13, lineHeight: 20 },
  izmirDistrictSectionTop: { marginTop: 28, marginBottom: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  izmirDistrictCount: { color: izmirTheme.gold, fontSize: 10, fontWeight: '900' },
  izmirFeaturedDistrictGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 14 },
  izmirFeaturedDistrictGridDesktop: { gap: 16 },
  izmirFeaturedDistrictCard: { width: '100%', minHeight: 164, overflow: 'hidden', borderRadius: 18, borderWidth: 1, borderColor: izmirTheme.lineGreen, backgroundColor: izmirTheme.forestDark, shadowColor: izmirTheme.forest, shadowOffset: { width: 0, height: 6 }, shadowOpacity: .10, shadowRadius: 12, elevation: 2 },
  izmirFeaturedDistrictCardTablet: { width: '48.8%' },
  izmirFeaturedDistrictCardDesktop: { width: '23.7%', minHeight: 174 },
  izmirFeaturedDistrictImage: { position: 'absolute', inset: 0, width: '100%', height: '100%' },
  izmirFeaturedDistrictShade: { position: 'absolute', inset: 0, ...(Platform.OS==='web'?({backgroundImage:'linear-gradient(180deg, rgba(16,47,40,.04) 20%, rgba(16,47,40,.88) 100%)'} as object):{backgroundColor:'rgba(16,47,40,.38)'}) },
  izmirFeaturedDistrictBody: { position: 'absolute', right: 42, bottom: 17, left: 17 },
  izmirFeaturedDistrictName: { color: izmirTheme.white, fontFamily: Platform.select({ios:'Georgia',android:'serif'}), fontSize: 20, fontWeight: '700' },
  izmirFeaturedDistrictTags: { marginTop: 5, color: izmirTheme.goldSoft, fontSize: 9, fontWeight: '800' },
  izmirFeaturedDistrictArrow: { position: 'absolute', right: 16, bottom: 19, color: izmirTheme.goldSoft, fontSize: 20, fontWeight: '900' },
  izmirAllDistrictsToggle: { minHeight: 48, marginTop: 18, paddingHorizontal: 18, alignSelf: 'flex-start', alignItems: 'center', justifyContent: 'center', borderRadius: 16, borderWidth: 1, borderColor: izmirTheme.lineGreen, backgroundColor: izmirTheme.paper },
  izmirAllDistrictsToggleText: { color: izmirTheme.forest, fontSize: 11, fontWeight: '900' },
  izmirAllDistrictsTitle: { marginTop: 34, marginBottom: 15, color: izmirTheme.forest, fontFamily: Platform.select({ios:'Georgia',android:'serif'}), fontSize: 25, fontWeight: '700' },
  izmirDistrictSearch: { height: 54, marginBottom: 12, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 10, borderRadius: 18, borderWidth: 1, borderColor: izmirTheme.lineGreen, backgroundColor: izmirTheme.paper },
  izmirDistrictFilterRail: { paddingBottom: 12, gap: 8 },
  izmirDistrictFilterChip: { minHeight: 42, paddingHorizontal: 14, alignItems: 'center', justifyContent: 'center', borderRadius: 15, borderWidth: 1, borderColor: izmirTheme.lineGreen, backgroundColor: izmirTheme.paper },
  izmirDistrictFilterChipActive: { borderColor: izmirTheme.forest, backgroundColor: izmirTheme.forest },
  izmirDistrictFilterText: { color: izmirTheme.forest, fontSize: 10, fontWeight: '900' },
  izmirDistrictFilterTextActive: { color: izmirTheme.white },
  izmirDistrictFilterResult: { marginBottom: 11, color: izmirTheme.muted, fontSize: 11, fontWeight: '800' },
  izmirAllDistrictsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  izmirAllDistrictsGridTablet: { gap: 12 },
  izmirAllDistrictsGridDesktop: { gap: 13 },
  izmirCompactDistrictCard: { width: '48.5%', minHeight: 58, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', gap: 9, borderRadius: 14, borderWidth: 1, borderColor: izmirTheme.line, backgroundColor: izmirTheme.paper },
  izmirCompactDistrictCardTablet: { width: '31.9%' },
  izmirCompactDistrictCardDesktop: { width: '19.1%' },
  izmirCompactDistrictIcon: { color: izmirTheme.moss, fontSize: 17, fontWeight: '900' },
  izmirCompactDistrictName: { flex: 1, color: izmirTheme.forest, fontSize: 12, fontWeight: '800' },
  izmirCompactDistrictArrow: { color: izmirTheme.gold, fontSize: 16, fontWeight: '900' },
  ...
  izmirFoodTab: { minWidth: 170, minHeight: 50, paddingHorizontal: 18, alignItems: 'center', justifyContent: 'center', borderRadius: 15, borderWidth: 1, borderColor: izmirTheme.lineGreen, backgroundColor: izmirTheme.paper },
  izmirFoodTabDesktop: { flex: 1, minHeight: 54 },
  izmirFoodTabActive: { borderColor: izmirTheme.forest, backgroundColor: izmirTheme.forest },
  izmirFoodTabText: { color: izmirTheme.forest, fontSize: 11, fontWeight: '900' },
  izmirFoodTabTextActive: { color: izmirTheme.white },
  izmirNightlifeSection: { marginTop: 4, paddingBottom: 8 },
  izmirNightlifeIntro: { marginTop: 8, maxWidth: 650, color: izmirTheme.muted, fontSize: 13, lineHeight: 20 },
  izmirNightlifeCard: { borderColor: izmirTheme.lineGreen, backgroundColor: izmirTheme.creamSoft },
  izmirTabBar: { height: 70, bottom: 8, borderColor: izmirTheme.moss, backgroundColor: izmirTheme.forest, shadowColor: izmirTheme.forestDark },
  izmirTabBarTablet: { height: 66, right: 26, bottom: 10, left: 26 },
  izmirTabInactive: { color: izmirTheme.lineGreen },
  izmirTabIndicator: { position: 'absolute', top: 3, width: 22, height: 3, borderRadius: 2, backgroundColor: izmirTheme.gold },
  izmirTabBadge: { backgroundColor: izmirTheme.gold },
