...
import { bursa, bursaDistricts, comingCities, type Category, type District, type Place } from './src/data';
...
export default function App() {
  ...
  const content = tab === 'plan'
    ? <PlanScreen districtNames={plannedDistricts} onRemove={toggleDistrictPlan} />
    : tab === 'favorites'
  ...
  return (
    <View style={styles.app}>
      ...
      {content}
      <BottomTabs tab={tab} setTab={setTab} favoriteCount={favorites.length} planCount={plannedDistricts.length} />
...
function MainContent({ exploreOnly, query, setQuery, category, setCategory, places, favorites, onFavorite, onOpen, onDistrictOpen }: {
  exploreOnly: boolean;
  query: string;
...
function PlanScreen({ districtNames, onRemove }: { districtNames: string[]; onRemove: (name: string) => void }) {
  ...
  const openRoute = () => {
    ...
    if (districts.length === 1) return Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(first.mapQuery)}`);
    const origin = encodeURIComponent(first.mapQuery);
    const destination = encodeURIComponent(last.mapQuery);
    const waypoints = districts.slice(1, -1).map(item => item.mapQuery).join('|');
