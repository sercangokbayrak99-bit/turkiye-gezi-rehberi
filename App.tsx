...
import { istanbulDistricts, istanbulPlaces, istanbulRoutes, istanbulVenueAreas } from './src/istanbulData';
...
function IstanbulGuide() {
  ...
  return <SafeAreaView style={styles.istanbulPage}><ScrollView contentContainerStyle={styles.istanbulContent} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.istanbulBody}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.istanbulDistrictRail}>{visibleDistricts.map((item, index) => <Pressable key={item.name} onPress={() => openMap(item.mapQuery)} style={[styles.istanbulDistrictCard, { backgroundColor: index % 3 === 0 ? '#315F53' : index % 3 === 1 ? '#75513B' : '#477A89' }]}><Text style={styles.istanbulDistrictSide}>{item.side.toUpperCase()}</Text><Text style={styles.istanbulDistrictName}>{item.name}</Text><Text style={styles.istanbulDistrictCopy}>{item.signature}</Text><Text style={styles.istanbulDistrictOpen}>Haritada aç  ↗</Text></Pressable>)}</ScrollView>
      {!!visiblePlaces.length && <><View style={styles.moduleHeading}><View><Text style={styles.istanbulEyebrow}>GERÇEK FOTOĞRAFLARLA</Text><Text style={styles.moduleTitle}>Öne çıkan keşifler</Text></View><Text style={styles.moduleHint}>{visiblePlaces.length} yer</Text></View><View style={styles.istanbulPlaceList}>{visiblePlaces.map(place => <View key={place.id} style={styles.istanbulPlaceCard}><Image source={place.image} style={styles.istanbulPlaceImage} /><View style={styles.istanbulPlaceBody}><Text style={styles.istanbulPlaceMeta}>{place.category.toUpperCase()} · {place.district.toUpperCase()}</Text><Text style={styles.istanbulPlaceName}>{place.name}</Text><Text style={styles.istanbulPlaceCopy}>{place.summary}</Text><View style={styles.istanbulPlaceActions}><Pressable onPress={() => openMap(place.mapQuery)} style={styles.istanbulMapButton}><Text style={styles.istanbulMapButtonText}>Konum ve yol tarifi</Text></Pressable><Pressable onPress={() => Linking.openURL(place.imagePage)}><Text style={styles.istanbulCredit}>Fotoğraf: {place.credit}</Text></Pressable></View></View></View>)}</View></>}
